...
/**
 * @notice ProofOfMovementInRange - Demonstrates token movement with IN-RANGE position
 * 
 * This script creates a position with ticks that INCLUDE the current pool tick,
 * which is the condition to trigger active liquidity management.
 * 
 * Expected flow:
 * 1. Position registered IN-RANGE
 * 2. Rebalance called
 * 3. Since position is IN-RANGE and ACTIVE, tokens stay in active pools
 * 4. Try marking position as IDLE to force movement back
 */
contract ProofOfMovementInRangeScript is Script {
    struct DepositInfo {
        address asset;
        address aToken;
        address aavePool;
    ...
    function run() external {
        ...
        // KEY CHANGE: Use IN-RANGE ticks instead of OUT-OF-RANGE
        (int24 lowerTick, int24 upperTick) = makeInRangeTicks(poolManager);
        console.log("=== IN-RANGE TICKS ===");
        ...
        console.log("lowerTick", lower);
        console.log("upperTick", upper);
        console.log("vaultShares0", vaultShares0);
        console.log("vaultShares1", vaultShares1);
        console.log("aTokenPrincipal0", aTokenPrincipal0);
        console.log("aTokenPrincipal1", aTokenPrincipal1);
        console.log("Hook tracked LPs", hook.getTrackedLPs(pid).length);
        console.log("Hook totalIdleLiquidity[0]", hook.totalIdleLiquidity(pid, 0));
        console.log("Hook totalIdleLiquidity[1]", hook.totalIdleLiquidity(pid, 1));

        console.log("=== EVENTS EMITTED ===");
        console.log("Total logs captured", logs.length);
        for (uint256 i = 0; i < logs.length; ++i) {
            console.log("Event", i, "topics:", logs[i].topics.length);
        }
    }
...
contract MockPoolManager {
    bytes32 private storageData = bytes32(uint256(42)); // some tick value
