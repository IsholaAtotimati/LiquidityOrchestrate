...
async function main() {
  ...
  for (let i = 0; i < iterations; i++) {
    // Swap token1 -> token0 (zeroForOne=false)
    try {
      const sp = [false, amountSpecified, 0];
      const tx = await pm.swap(poolKey, sp, '0x');
      await tx.wait();
      console.log(`reverse swap #${i} executed`);
    } catch (err) {
      console.error('swap failed:', err.message || err);
    }

    // Stop when upkeep cleared
    try {
      const [need] = await hook.checkUpkeep('0x');
      if (!need) { console.log('hook no longer needs update — stopping'); break; }
    } catch (e) {
      console.log('checkUpkeep failed (non-fatal):', e.message || e);
    }
  }
}
